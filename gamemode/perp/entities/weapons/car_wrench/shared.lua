...
function SWEP:PrimaryAttack()	
	...
	if CLIENT then
		if(IsValid(EyeTrace.Entity) and EyeTrace.Entity:IsVehicle()) then
			for i=0, 1.3, 0.3 do
		...
		if(IsValid(EyeTrace.Entity) and EyeTrace.Entity:IsVehicle() and EyeTrace.Entity:GetTable().CarDamage) then

			if(EyeTrace.Entity:GetTable().CarDamage > 49 and EyeTrace.Entity.Disabled) then
				EyeTrace.Entity:FixCar()
			else
