...
SWEP.NormalPos = Vector (-0.685, 7.1894, 1.9321)
...
function SWEP:TryToBatter ( Target )
	...
	if Randomness <= self.PercentChange then
		...
		Target:EmitSound(self.BatterFailSound);
	end
end
...
function SWEP:PrimaryAttack()
	...
	if (!doorOwner || !IsValid(doorOwner)) then
		...
		if doorOwner:GetSharedBool("warrent", false) then
			self:TryToBatter(EyeTrace.Entity);
		else
...
function SWEP:SecondaryAttack()
	...
	if EyeTrace.Entity:GetDriver():GetSharedBool("warrent", false) then
