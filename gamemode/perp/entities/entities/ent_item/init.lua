...
function ENT:Use( activator, caller )
	...
	if (self:GetTable().ItemID == 9 && self:GetSharedBool("smoking", false)) then
		if ((CurTime() - self.smokeStart) >= METH_BURN_TIME) then
			activator:GiveItem(11, 1, true);
		else
			activator:GiveItem(10, 1, true);
		end
	elseif (self:GetSharedString("title", "") != "" and self:GetTable().Owner and activator != self:GetTable().Owner) then
		local ourPrice = self:GetSharedInt("price", -1)
		...
		self:GetTable().Owner:Notify(activator:GetRPName() .. " bought your '" .. self:GetSharedString("title", "") .. "'. You earned $" .. winCash .. " (" .. GAMEMODE.GetTaxRate_Sales_Text() .. " Sales Tax)")
...
function ENT:SetContents ( ItemID, Owner )
	...
	self:GetTable().ItemSpawner = Owner
end
...
function ENT:Think ( )
	...
	if (self:GetTable().ItemID && self:GetTable().ItemID == 9) then
		...
		if (nearHeatSource) then
			...
			if (self.willBurn && (CurTime() - self.smokeStart) >= (METH_COOK_TIME * .25)) then				
				...
				self:Explode();
			elseif ((CurTime() - self.smokeStart) >= METH_COOK_TIME && !self:GetSharedBool("smoking", false)) then
				self:SetSharedBool("smoking", true);
	...
	if (self:GetSharedString("title", "") != "" && !self.pushedOBBMaxs) then
		local maxs = self:OBBMaxs()
		local mins = self:OBBMins()
		
		self:SetSharedVector("maxs", Vector((mins.x + maxs.x) * 0.5, (mins.y + maxs.y) * 0.5, maxs.z))
	end
end
...
function ENT:StartTouch ( Ent )
	...
	if (self:GetSharedString("title", "") != "") then return end
	...
	if (toMake.Results == 13) then		
		...
		newItem.maker = oldOwner:SteamID();
		newItem:GetTable().ItemSpawner = oldOwner
		newItem:GetTable().Owner = oldOwner
