...
function ENT:Initialize()
	...
	self.GrowthTime = COCA_GROW_TIME
...
function ENT:Use( activator, caller )
	...
	if activator:Team() == TEAM_POLICE || activator:Team() == TEAM_SWAT then
		...
		activator:PrintMessage(HUD_PRINTTALK, "You have been rewarded $" .. GAMEMODE.CopReward_Arrest .. " for destroying cocaine.")
		self:Remove()
	...
	elseif self.dt.SpawnTime + self.GrowthTime < CurTime() then		
		...
		if(not activator:CanHoldItem(69, roll)) then
			activator:Notify("You don't have enough inventory room!")
			return
		end
		
		if(not activator:CanHoldItem(68, sroll)) then
			activator:Notify("You don't have enough inventory room!")
			return
		end
		
		activator:GiveItem(69, roll, true)
		
		self.Entity:GetTable().Tapped = true
		
		if (sroll != 0) then
			activator:GiveItem(68, sroll, true)
		end
	...
	elseif self.dt.SpawnTime + self.GrowthTime * .5 < CurTime() then		
		...
		if roll == 0 then
			activator:Notify("You nearly salvaged some, but you picked too early to get any usable cocaine.")
		else
			activator:Notify("You picked too early for any seeds to be found, but some cocaine was usable.")
		end
