...
function ENT:HitByExtinguisher ( ByWho, IsHose )
	...
	if self.ExtinguisherLeft <= 0 then
		if ByWho and ByWho:IsValid() and ByWho:IsPlayer() then
			if ByWho:Team() == TEAM_FIREMAN then
				if IsHose then
					ByWho:GiveCash(5);
				...
				ByWho:Notify("Fire extinguished!");
			end
		end
...
function ENT:Think ( )
	if self.Spores >= 120 then self:KillFire(); return false; end
	...
	if IsStorming and IsOutdoors then
		...
		if (self.LastSpread + 20 < CurTime() and (!IsRaining or !IsOutdoors)) or (self.LastSpread + 60 < CurTime() and (IsRaining and IsOutdoors)) then
			self.Spores = self.Spores + 1;
			self:SpreadFire();
