...
function PANEL:Paint ( )
	...
	elseif (self.level == 2) then
		...
		for k, v in pairs(player.GetAll()) do
			if (v != LocalPlayer() && v:GetSharedInt("org", 0) == LocalPlayer():GetSharedInt("org", 0) && v:GetSharedInt("org", 0) != 0) then
	...
	for i = self.startPos, self.startPos + 4 do
		if (self.toDisplayStuff[i]) then
			...
			if (self.level == 4 && self.SpotToRing[i] == LocalPlayer():GetSharedInt("ringtone", 1)) then prefix = "> "; end
