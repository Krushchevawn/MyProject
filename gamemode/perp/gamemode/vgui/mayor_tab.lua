...
function PANEL:AddConVarBar ( parent, name, default, min, max, conVar, upGM )
	...
	newBar.ValueChanged = function(pSelf, pValue)
		pSelf:SetValue(math.Round(pValue))
	end
...
function PANEL:Think ( )
	...
	for _, each in pairs(player.GetAll()) do
		...
		if(each:IsAdmin() or each:IsOwner()) then
			TO_EARN = TO_EARN * 2
		elseif(each:IsGoldMember()) then
			TO_EARN = TO_EARN * 1.6
		elseif (each:IsVIP()) then
			TO_EARN = TO_EARN * 1.3
		end
...
function PANEL:Init ( )
	...
	self.warrentList:EnableVerticalScrollbar(true)
	...
	self.officialsList:EnableVerticalScrollbar(true)
	...
	self.taxesList:EnableVerticalScrollbar(true)
	self.taxesList:SetPadding(5);
	self.taxesList:SetSpacing(5);
	
	self.ann = vgui.Create("DButton", self.taxesList);
	self.ann:SetText("Announce new mayor changes.");
	self.ann.DoClick = function() RunConsoleCommand("perp_m_ann_ch") end
	self.taxesList:AddItem(self.ann);
	...
	self:AddConVarBar(self.taxesList, "Sales Tax Rate ( Be careful - this could aggrivate citizens! )", GetSharedInt("tax_sales", 0), 0, GAMEMODE.MaxTax_Sales, "perp_m_t_s")
	self:AddConVarBar(self.taxesList, "Income Tax Rate ( Be careful - this could aggrivate citizens! )", GetSharedInt("tax_income", 0), 0, GAMEMODE.MaxTax_Income, "perp_m_t_i")
	
	self:AddConVarBar(self.taxesList, "Traffic Ticket Price ( Be careful - this could aggrivate citizens! )", GetSharedInt("ticket_price", 0), 0, GAMEMODE.MaxTicketPrice, "perp_m_ticket_p")
	
	// Law Enforcement
	self.lawEnforcementList = vgui.Create("DPanelList", self)
	self.lawEnforcementList:EnableVerticalScrollbar(true)
	self.lawEnforcementList:SetPadding(5);
	self.lawEnforcementList:SetSpacing(5);
	
	self.ann2 = vgui.Create("DButton", self.lawEnforcementList);
	self.ann2:SetText("Announce new mayor changes.");
	self.ann2.DoClick = function() RunConsoleCommand("perp_m_ann_ch") end
	self.lawEnforcementList:AddItem(self.ann2);
	...
	self:AddConVarBar(self.lawEnforcementList, "Inner City Speed Limit (MPH)", GAMEMODE.GetInnerCitySpeedLimit(), 20, 120, "perp_m_sl_i", "MaxInnerCitySpeedLimit")
	self:AddConVarBar(self.lawEnforcementList, "Outter City Speed Limit (MPH)", GAMEMODE.GetOutterCitySpeedLimit(), 20, 120, "perp_m_sl_o", "MaxOutterCitySpeedLimit")
	...
	self:AddConVarBar(self.lawEnforcementList, "Police Officer Salary ( The amount officers are paid every pay day. )", GAMEMODE.PayDay_Police, GAMEMODE.MinimumPayGov, GAMEMODE.MaxPayDay_Police, "perp_m_p_c", "PayDay_Police")
	...
	self:AddConVarBar(self.lawEnforcementList, "SWAT Member Salary ( The amount officers are paid every pay day. )", GAMEMODE.PayDay_SWAT, GAMEMODE.MinimumPayGov, GAMEMODE.MaxPayDay_SWAT, "perp_m_p_s", "PayDay_SWAT")
	self:AddConVarBar(self.lawEnforcementList, "SWAT Van Upkeep ( How many SWAT vans are available. $" .. GAMEMODE.UpkeepCost_SWATVan .. " upkeep each. )", GAMEMODE.MaxSWATVans, 0, GAMEMODE.MaxVehicles_SWAT, "perp_m_v_s", "MaxSWATVans")
	
	self:AddConVarBar(self.lawEnforcementList, "Dispatcher Salary ( The amount the dispatcher is paid every pay day. )", GAMEMODE.PayDay_Dispatcher, GAMEMODE.MinimumPayGov, GAMEMODE.MaxPayDay_Dispatcher, "perp_m_p_d", "PayDay_Dispatcher")
	
	// Civil Services
	self.civilServicesList = vgui.Create("DPanelList", self)
	self.civilServicesList:EnableVerticalScrollbar(true)
	self.civilServicesList:SetPadding(5);
	self.civilServicesList:SetSpacing(5);
	
	self.ann3 = vgui.Create("DButton", self.civilServicesList);
	self.ann3:SetText("Announce new mayor changes.");
	self.ann3.DoClick = function() RunConsoleCommand("perp_m_ann_ch") end
	self.civilServicesList:AddItem(self.ann3);
	...
	self:AddConVarBar(self.civilServicesList, "Fireman Salary ( The amount firemen are paid every pay day )", GAMEMODE.PayDay_Fireman, GAMEMODE.MinimumPayGov, GAMEMODE.MaxPayDay_Fireman, "perp_m_p_f", "PayDay_Fireman")
	...
	self:AddConVarBar(self.civilServicesList, "Paramedic Salary ( The amount paramedics are paid every pay day )", GAMEMODE.PayDay_Medic, GAMEMODE.MinimumPayGov, GAMEMODE.MaxPayDay_Paramedic, "perp_m_p_m", "PayDay_Medic")
	...
	self.secretServiceList:EnableVerticalScrollbar(true)
	...
	self:AddConVarBar(self.secretServiceList, "Secret Service Agent Salary ( The amount Secret Service agents are paid every pay day )", GAMEMODE.PayDay_SecretService, GAMEMODE.MinimumPayGov, GAMEMODE.MaxPayDay_SecretService, "perp_m_p_ss", "PayDay_SecretService")
