...
function PANEL:Paint()
	...
	draw.SimpleText( self.Text, "consoletext", self:GetWide() / 2, self:GetTall() / 2, Color(0,0,0,150), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
...
function PANEL:DoCommand( ply )
	...
	ShowGetBanTime("Kicking " .. ply:Nick(), "Kick Reason", "Kick Player", function ( t )
		if (ply && IsValid(ply) && ply:IsPlayer()) then
			RunConsoleCommand("Ass_KickPlayer", ply:UniqueID(), t);
		end
	end);
...
function PANEL:DoCommand( ply )
	...
	RunConsoleCommand( "Ass_KillPlayer", ply:UniqueID() )
...
function PANEL:DoCommand( ply )
	...
	ShowGetBanTime("Perma Banning " .. ply:Nick(), "Ban Reason", "Ban Player", function ( t )
		if (ply && IsValid(ply) && ply:IsPlayer()) then
			RunConsoleCommand("Ass_BanPlayer", ply:UniqueID(), "0", t);
		end
	end);
...
function PANEL:DoCommand( ply )
	...
	ShowGetBanTime("Banning " .. ply:Nick(), "Ban Reason", "Ban Player", function ( t )
		if (ply && IsValid(ply) && ply:IsPlayer()) then
			RunConsoleCommand("Ass_BanPlayer", ply:UniqueID(), self:GetParent().scrollerTime:GetValue() * 60, t);
		end
	end);
