...
function PLAYER:Notify ( Text, truefalse )
	...
	if SERVER then
		...
		GAMEMODE.AddNotification(Text, NOTIFY_GENERIC, 15, truefalse);
	end
end

function PLAYER:GetRPName ( )
	return self:GetSharedString("rp_fname", "John") .. " " .. self:GetSharedString("rp_lname", "Doe");
end

function PLAYER:GetFirstName ( )
	return self:GetSharedString("rp_fname", "John");
end

function PLAYER:GetLastName ( )
	return self:GetSharedString("rp_lname", "Doe");
end
...
function PLAYER:AddFuel ( value, stopSendToClient) self:SetPrivateInt("fuelleft", self:GetFuel() + value, stopSendToClient); end
function PLAYER:TakeFuel ( value, stopSendToClient) self:SetPrivateInt("fuelleft", self:GetFuel() - value, stopSendToClient); end
...
function PLAYER:CalculateStaminaLoss ()
	...
	/*if (self:GetSharedInt("typing", nil) && realSpeed >= 20 && self.StartedTyping && self.StartedTyping + 2 < CurTime()) then
		self.StartedTyping = nil;
		self:SetSharedInt("typing", nil)
	end*/
	...
	elseif (self:OnGround() || self:InVehicle() || !self:OnGround()) then
		...
		if (self.LastStaminaAdd + (GAMEMODE.SprintDecay * 6) <= CurTime()) then
			...
			if (SERVER && self.Stamina >= 26 && self:GetSharedInt("tired", 0) == 1) then
				self:SetSharedInt("tired", nil);
			end
