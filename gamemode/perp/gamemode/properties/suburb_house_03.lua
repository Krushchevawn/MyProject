


PROPERTY = {};

PROPERTY.ID = 41;

PROPERTY.Name = "Suburbs House #3";
PROPERTY.Category = "House";
PROPERTY.Description = "A moderately sized house in the suburbs.";
PROPERTY.Image = "ev3x_subs3";

PROPERTY.Cost = 3000;

PROPERTY.Doors = 	{
						{Vector(5101, 13657.875, 144), 'models/props_c17/door01_left.mdl'},
						{Vector(5483.5, 14118, 111.09375), '*20'},
						{Vector(4571, 13817, 144), 'models/props_c17/door01_left.mdl'},
						{Vector(4671, 13889, 144), 'models/props_c17/door01_left.mdl'},
						{Vector(4759, 13817, 144), 'models/props_c17/door01_left.mdl'},
						{Vector(5171, 13753, 144), 'models/props_c17/door01_left.mdl'},
						{Vector(5274, 13872, 144), 'models/props_c17/door01_left.mdl'},
					};
					
GAMEMODE:RegisterProperty(PROPERTY);