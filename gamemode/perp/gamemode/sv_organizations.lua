...
function GM.FetchOrganizationData ( orgID )
	...
	DataBase:Query("SELECT `name`, `motd`, `owner` FROM `perp_organization` WHERE `id`='" .. orgID .. "' LIMIT 1", function ( orgInfo )
		if (orgInfo[1] && orgInfo[1][1]) then
			local name = orgInfo[1][1];
			local motd = orgInfo[1][2];
			local ownerID = orgInfo[1][3];
			
			DataBase:Query("SELECT `rp_name_first`, `rp_name_last` FROM `perp_users` WHERE `id`='" .. ownerID .. "' LIMIT 1", function ( ownerInfo ) 
	...
	DataBase:Query("SELECT `rp_name_first`, `rp_name_last`, `uid` FROM `perp_users` WHERE `organization`='" .. orgID .. "'", function ( memData )
		if (memData[1]) then
...
function GM.UpdateOrganizationData ( Player, Cmd, Args )
	...
	local playerOrg = Player:GetSharedInt("org", 0);
	...
	DataBase:Query("UPDATE `perp_organization` SET `name`='" .. tmysql.escape(newName) .. "', `motd`='" .. tmysql.escape(newMOTD) .. "' WHERE `id`='" .. playerOrg .. "' LIMIT 1");
...
function GM.InviteToOrganization ( Player, Cmd, Args )
	...
	if (sPlayer:GetSharedInt("org", 0) != 0) then return; end
...
function GM.KickFromOrganization ( Player, Cmd, Args )
	...
	if (!sPlayer) then return; end
	if (sPlayer:GetSharedInt("org", 0) != playerOrg) then return; end
	
	sPlayer:Notify("You have been kicked out of " .. GAMEMODE.OrganizationData[playerOrg][1] .. ".");
	sPlayer:SetSharedInt("org", nil);
	
	DataBase:Query("UPDATE `perp_users` SET `organization`='0' WHERE `id`='" .. sPlayer.SMFID .. "'");
...
function GM.AcceptInvitation ( Player )
	if (Player:GetSharedInt("org", 0) != 0) then return; end
	if (!Player.invitedTo || !Player.invitedBy) then return; end
	
	Player:SetSharedInt("org", Player.invitedTo);
	
	if (Player.invitedBy && IsValid(Player.invitedBy)) then
		Player.invitedBy:Notify(Player:GetRPName() .. " has accepted your invitation.");
	end
	
	DataBase:Query("UPDATE `perp_users` SET `organization`='" .. Player.invitedTo .. "' WHERE `id`='" .. Player.SMFID .. "'");
...
function GM.LeaveOrganization ( Player )
	...
	local orgID = Player:GetSharedInt("org", 0);
	...
	if (tostring(Player.SMFID) == tostring(GAMEMODE.OrganizationData[orgID][4])) then		
		for k, v in pairs(player.GetAll()) do
			if (v:GetSharedInt("org", 0) == orgID) then
				if (v != Player) then
					v:Notify(GAMEMODE.OrganizationData[orgID][1] .. " has been disbaned.");
				end
				
				v:SetSharedInt("org", 0);
			end
		end
		
		DataBase:Query("UPDATE `perp_users` SET `organization`='0' WHERE `organization`='" .. orgID .. "'");
		DataBase:Query("DELETE FROM `perp_organization` WHERE `id`='" .. orgID .. "'");
	else		
		Player:SetSharedInt("org", 0);
		DataBase:Query("UPDATE `perp_users` SET `organization`='0' WHERE `id`='" .. Player.SMFID .. "'");
