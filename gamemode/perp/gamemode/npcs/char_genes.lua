...
NPC.Model = Model(PERPGetModelPath(1, 4, 1))
...
function NPC.Cost ( )
	...
	if LocalPlayer():IsGoldMember() then
		cost = (((GAMEMODE.MaxGoldGenes - 5) - (GAMEMODE.MaxGoldGenes - LocalPlayer():GetNumGenes())) + 1) * GAMEMODE.NewGenePrice
	else
		cost = (((GAMEMODE.MaxGenes - 5) - (GAMEMODE.MaxGenes - LocalPlayer():GetNumGenes())) + 1) * GAMEMODE.NewGenePrice
	end
...
function NPC.Add ( )
	...
	if LocalPlayer():GetBank() >= cost then
		if (LocalPlayer():IsGoldMember() and LocalPlayer():GetNumGenes() < GAMEMODE.MaxGoldGenes) or (!LocalPlayer():IsGoldMember() and LocalPlayer():GetNumGenes() < GAMEMODE.MaxGenes) then
			GAMEMODE.DialogPanel:SetDialog("<5 Hours Later> There you go, sir. Enjoy your new mastery of any field desirable.");
			
			RunConsoleCommand('perp_buy_gene');
