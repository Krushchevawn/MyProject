...
//NPC.Model = Model(PERPGetModelPath(2, 2, 2))
...
function NPC.RunForMayor ( )
	...
	*/
	...
	if(GAMEMODE.VoteIsGoing) then
		GAMEMODE.DialogPanel:SetDialog("There is already a mayor vote going on.");
...
function NPC.AskAboutSS ( )
	...
	elseif (LocalPlayer():HasBlacklist(GAMEMODE.teamToBlacklist[TEAM_SECRET_SERVICE])) then
		...
		GAMEMODE.DialogPanel:AddDialog("Oh, right...", LEAVE_DIALOG);	
	elseif (LocalPlayer():GetTimePlayed() < GAMEMODE.RequiredTime_SecretService * 60 * 60 && !LocalPlayer():IsVIP()) then
...
function NPC.GrabLimo ( )
	...
	for k, v in pairs(ents.FindByClass("prop_vehicle_jeep")) do
		...
		if (vT && vT.RequiredClass == TEAM_SECRET_SERVICE && v:GetSharedEntity("owner", nil) != LocalPlayer()) then
