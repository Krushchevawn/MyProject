...
NPC.Model = Model(PERPGetModelPath(1, 4, 7))


NPC.Invisible = false; // Used for ATM Machines, Casino Tables, etc.

NPC.Location = Vector(-9512.3994, 9194.3428, 72.0313);
NPC.Angles = Angle(0, 90.4913, 0.000000);
...
function NPC.Hire ( )
	...
	elseif (LocalPlayer():GetTimePlayed() < GAMEMODE.RequiredTime_Paramedic * 60 * 60 && !LocalPlayer():IsVIP()) then
...
function NPC.Ambulance ( )
	...
	for k, v in pairs(ents.FindByClass("prop_vehicle_jeep")) do
		...
		if (vT && vT.RequiredClass == TEAM_MEDIC && v:GetSharedEntity("owner", nil) != LocalPlayer()) then
