...
function NPC.OnTalk ( )
	...
	for k, v in pairs(ents.FindByClass("prop_vehicle_jeep")) do
		if (v:GetSharedInt("carid") == tonumber(CID)) then
			ourVehicle = v;
					VOwner = ourVehicle:GetSharedEntity("owner");
					if VOwner == nil then return end;
					CurFuel = VOwner:GetFuel()
...
if (!ourVehicle || !IsValid(ourVehicle) || ourVehicle:GetPos():Distance(LocalPlayer():GetPos()) > 300) then
	...
	local CashToTake = math.Round(ammount * .03)
...
if FuelMax - CurFuel < 2500 then local Total = FuelMax - CurFuel;
	NPC.Gas(Total);
	else
	NPC.Gas(2500);
	end
...
function NPC.Half ( ) 
if FuelMax - CurFuel < 5000 then local Total = FuelMax - CurFuel;
	NPC.Gas(Total);
	else
	NPC.Gas(5000);
	end
