...
net.Receive( "perp_vehicles_init", function( length, client )
	ParseVehicleString(net.ReadString())
end )
...
function DoFuel ( )
	if LocalPlayer() and LocalPlayer():InVehicle() and LocalPlayer():GetVehicle():GetSharedBool("Started", false) then
		local Tbl = LocalPlayer():GetVehicle().vehicleTable;
		if Tbl == nil then return end;
		if !Tbl.DF then return end;
		ClientCar = LocalPlayer():GetVehicle():GetSharedInt("carid");
...
hook.Add("InitPostEntity", "Initfueltimer", function()
	timer.Create("DoFuel", 3, 0, function() DoFuel() end)
end)
