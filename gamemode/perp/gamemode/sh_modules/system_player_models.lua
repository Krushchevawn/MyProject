...
MODEL_AVAILABLE["f"][3] = {1, 2, 4, 5}; //{1, 2, 3, 4, 5}; -Exploit with model 3
...
function PLAYER:GetModelInfo ( )
	local info = string.gsub(string.gsub(string.GetFileFromFilename(self:GetPrivateString("model", self:GetModel())), ".mdl", ""), "/", "")
	if(string.find(info, "male")) then
		info = info .. "_08"
		string.gsub(info, "male", "m")
	end
	return GAMEMODE.ExplodeModelInfo(info)
end
...
function PERPGetModelPath(sex, face, clothes)
...
function PLAYER:GetModelPath ( sex, face, clothes ) return PERPGetModelPath(sex, face, clothes) end