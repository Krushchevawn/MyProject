	...
	for k, v in pairs(player.GetAll()) do
		if (v.ringing) then
			if (!v.lastRingTime || v.lastRingTime < CurTime()) then
				if (!v.ringer) then
					v.ringer = CreateSound(v, Sound("PERP2.5/ringtones/" .. GAMEMODE.Ringtones[v:GetSharedInt("ringtone", 1)][2]));
				end
								
				v.ringer:Stop();
				v.ringer:Play();
				
				v.lastRingTime = CurTime() + SoundDuration("PERP2.5/ringtones/" .. GAMEMODE.Ringtones[v:GetSharedInt("ringtone", 1)][2]) + 1;
			end
