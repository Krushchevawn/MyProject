...
local textFilter = {}
textFilter["lol"] = "Please do not use acronyms in RP chat."
textFilter["rofl"] = "Please do not use acronyms in RP chat."
textFilter["ftw"] = "Please do not use acronyms in RP chat."
textFilter["ftl"] = "Please do not use acronyms in RP chat."
textFilter["pwn"] = "Please do not use acronyms in RP chat."
textFilter["lmao"] = "Please do not use acronyms in RP chat."
textFilter["lawl"] = "Please do not use acronyms in RP chat."
textFilter["asl"] = "Please do not use acronyms in RP chat."
...
function GM:PlayerSay ( Player, Text, TeamChat )
	...
	if (DeadPlayer) then
		if string.Left( string.Trim(Text), 4 ) != "/ooc" and string.Left( string.Trim(Text),  2 ) != "//"  and string.Left( string.Trim(Text), 3 ) != "///" and string.Left( string.Trim(Text), 5 ) != "/looc" and string.Left( string.Trim(Text), 7 ) != "/report" then
	...
	for k, v in pairs(chatPatterns) do
		if (string.match(string.lower(Text), "^[ \t]*[/!]" .. string.lower(k))) then
			if (!v[4] || v[4](Player, Text)) then
				...
				if (k != "a") then GAMEMODE.AddChatLog(toSend); end
...
chatPatterns["ooc"] = {"perp_ochat", 2, 
						function ( from, to ) return from:IsModerator() or GAMEMODE.AllowOOC end, 
						nil, 
						nil
					};
chatPatterns["/"] = chatPatterns["ooc"];

chatPatterns["a"] = {"perp_ochat", 13, 
...
chatPatterns["event"] = {"perp_fchat", 16, 
						function ( from, to ) return from:GetPos():Distance(to:GetPos()) <= ChatRadius_Local || (to:GetTable().Spectating && to:GetTable().Spectating:IsValid() && to:GetTable().Spectating:IsPlayer() && to:GetTable().Spectating:GetPos():Distance(from:GetPos()) <= ChatRadius_Local) end, 
						nil, 
...
chatPatterns["org"] = {"perp_chat", 10, 
						function ( from, to ) return from:GetSharedInt("org", 0) == to:GetSharedInt("org", 0) && to:HasItem("item_phone") end, 
						function ( pl )
							if (pl:GetSharedInt("org", 0) == 0) then
