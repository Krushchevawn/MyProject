...
function GM.Swat_Join ( Player )
	...
	if ((Player:GetTimePlayed() < GAMEMODE.RequiredTime_SWAT * 60 * 60 or !Player:IsVIP()) and !Player:IsAdmin() ) then return end
...
function GM.Swat_Quit ( Player )
	if (!Player:NearNPC(11) or Player:Team() != TEAM_SWAT) then return; end
...
function GM.Swat_Leave ( Player )
	...
	if (ammoTheyHave > 0) then
		Player:RemoveAmmo(ammoTheyHave + 30, 'smg1');
	end
...
function GM.Swat_SpawnCar ( Player )
	...
	for k, v in pairs(ents.FindByClass("prop_vehicle_jeep")) do		
		if (v.vehicleTable && v.vehicleTable.RequiredClass == TEAM_SWAT && v:GetSharedEntity("owner", nil) != Player) then
