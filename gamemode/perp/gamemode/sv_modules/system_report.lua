...
local function finishReport ( SMFID, rpName, ruleID, reporterID, reporterName, comments )
	...
	DataBase:Query("INSERT INTO `smf_messages` (`ID_BOARD`, `posterTime`, `ID_MEMBER`, `subject`, `posterName`, `posterEmail`, `posterIP`, `body`) VALUES ('13', '" .. addedTime .. "', '" .. reporterID .. "', '" .. tmysql.escape(rpName) .. " - Rule #" .. ruleID .. "', '" .. reporterName .. "', 'automated@perp.com', 'Not Available', '" .. body .. "')", function ( res )
		DataBase:Query("SELECT `ID_MSG` from `smf_messages` WHERE `ID_MEMBER`='" .. reporterID .. "' AND `posterTime`='" .. addedTime .. "' LIMIT 1", function ( res2 )
			if (!res2 || !res2[1] || !res2[1][1]) then Msg("no res2\n"); return; end
			
			local msgID = res2[1][1];
			
			DataBase:Query("INSERT INTO `smf_topics` (`ID_BOARD`, `ID_FIRST_MSG`, `ID_LAST_MSG`, `ID_MEMBER_STARTED`, `numReplies`, `numViews`) VALUES ('13', '" .. msgID .. "', '" .. msgID .. "', '" .. reporterID .. "', '0', '0')", function ( res3 )
				DataBase:Query("SELECT `ID_TOPIC` from `smf_topics` WHERE `ID_FIRST_MSG`='" .. msgID .. "' LIMIT 1", function ( res4 )
					if (!res4 || !res4[1] || !res4[1][1]) then Msg("no res3\n"); return; end
					
					local topicID = res4[1][1];
					
					DataBase:Query("UPDATE `smf_messages` SET `ID_TOPIC`='" .. topicID .. "' WHERE `ID_MSG`='" .. msgID .. "' LIMIT 1");
					DataBase:Query("UPDATE `smf_boards` SET `numTopics`=`numTopics`+'1', `numPosts`=`numPosts`+'1', `ID_LAST_MSG`='" .. msgID .. "', `ID_MSG_UPDATED`='" .. msgID .. "' WHERE `ID_BOARD`='13' LIMIT 1");
				end);
			end);
		end);
	end);
...
function GM.ReportPlayer ( Player, Cmd, Args )
	...
	DataBase:Query("SELECT `id`, `rp_name_first`, `rp_name_last` FROM `perp_users` WHERE `uid`='" .. uniqueID .. "' LIMIT 1", function ( res )
