...
function GM.Fireman_Join ( Player )
	...
	if (Player:GetTimePlayed() < GAMEMODE.RequiredTime_Fire * 60 * 60 && !Player:IsVIP()) then return end
...
function GM.Fireman_SpawnCar ( Player )
	...
	for k, v in pairs(ents.FindByClass("prop_vehicle_jeep")) do		
		if (v.vehicleTable && v.vehicleTable.RequiredClass == TEAM_FIREMAN && v:GetSharedEntity("owner", nil) != Player) then
