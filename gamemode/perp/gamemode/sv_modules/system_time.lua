...
//  This content was based  //
// on work created by other //
//   authors with consent.  //
///////////////////////////////
...
function GM.SaveDate ( )
	...
	if (GAMEMODE.LastSaveYear != GAMEMODE.CurrentYear) then
		DataBase:Query("UPDATE `perp_system` SET `value`='" .. GAMEMODE.CurrentYear .. "' WHERE `key`='date_year_" .. GAMEMODE.ServerDateIdentifier .. "' LIMIT 1");
	end
	
	if (GAMEMODE.LastSaveMonth != GAMEMODE.CurrentMonth) then
		DataBase:Query("UPDATE `perp_system` SET `value`='" .. GAMEMODE.CurrentMonth .. "' WHERE `key`='date_month_" .. GAMEMODE.ServerDateIdentifier .."' LIMIT 1");
	end
	
	if (GAMEMODE.LastSaveDay != GAMEMODE.CurrentDay) then
		DataBase:Query("UPDATE `perp_system` SET `value`='" .. GAMEMODE.CurrentDay .. "' WHERE `key`='date_day_" .. GAMEMODE.ServerDateIdentifier .."' LIMIT 1");
	end
...
local function setupTime ( )
	...
	for _ , brush in pairs(GAMEMODE.timeEntities.sky_overlay) do
		brush:SetRenderMode(RENDERMODE_TRANSALPHA)
		brush:Fire('Enable' , '' , 0);
