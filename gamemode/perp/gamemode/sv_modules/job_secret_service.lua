...
function GM.Secret_Service_Join ( Player )
	...
	if (Player:GetTimePlayed() < GAMEMODE.RequiredTime_SecretService * 60 * 60 && !Player:IsVIP()) then return end
...
function GM.Secret_Service_SpawnCar ( Player )
	...
	for k, v in pairs(ents.FindByClass("prop_vehicle_jeep")) do		
		if (v.vehicleTable && v.vehicleTable.RequiredClass == TEAM_SECRET_SERVICE && v:GetSharedEntity("owner", nil) != Player) then
