...
local function catchUnderglowPress ( Player )
	...
	if(!owner) then owner = Player:GetVehicle():GetSharedEntity("owner") end
...
local function catchFlashlightPress ( Player )
	...
	if (!Player:GetVehicle().Headlights) then
		...
		for k, v in pairs(Player:GetVehicle().vehicleTable.HeadlightPositions) do						
			local flashlight = ents.Create("env_projectedtexture");
				...
				flashlight:SetKeyValue("nearz", 65);
				flashlight:SetKeyValue("lightfov", 50);
				flashlight:SetKeyValue("LightWorld", 1)		
	...
	if (Player:GetVehicle().HeadlightsOn) then
		...
		Player:GetVehicle():SetSharedBool("hl", false);
	else
		for k, v in pairs(Player:GetVehicle().Headlights) do
			v:Fire("TurnOn", "", 0);
			v.StatusOn = true;
		...
		Player:GetVehicle():SetSharedBool("hl", true);
	end
end
...
local function flashHeadlights ( )
	for k, v in pairs(ents.FindByClass("prop_vehicle_jeep")) do
		if (v.vehicleTable and v.vehicleTable.ID == 'z' && v:GetSharedBool("slights", false)) then
			...
			if (v:GetSharedInt("fl", 0) != tV) then
				v:SetSharedInt("fl", tV);
			end
			
			if (v.Headlights && v:GetSharedBool("hl", false)) then
		...
		elseif (v:GetSharedInt("fl", 0) != 0) then
			v:SetSharedInt("fl", 0);
			
			if (v.Headlights) then
				local shouldBeOn_1 = v:GetSharedBool("hl", false);
				local shouldBeOn_2 = v:GetSharedBool("hl", false);
...
local function catchHyd ( Player )
	...
	if (vehicleTable.SirenNoise) then
		Player:GetVehicle():SetSharedBool("slights", !Player:GetVehicle():GetSharedBool("slights", false));
...
function GM.ToggleSiren ( Player )
	...
	Player:GetVehicle():SetSharedBool("siren", !Player:GetVehicle():GetSharedBool("siren", false));
end
...
local function manageLoudSirens ( )
	for k, v in pairs(ents.FindByClass("prop_vehicle_jeep")) do
		if (v.vehicleTable && v:GetDriver() && IsValid(v:GetDriver()) && v:GetDriver():IsPlayer() && v.vehicleTable.SirenNoise_Alt) then
			if (v:GetDriver():KeyDown(IN_WALK) && !v:GetSharedBool("siren_loud", false)) then
				v:SetSharedBool("siren_loud", true);
			elseif (!v:GetDriver():KeyDown(IN_WALK) && v:GetSharedBool("siren_loud", false)) then
				v:SetSharedBool("siren_loud", false);
			end
		end
	end
end
