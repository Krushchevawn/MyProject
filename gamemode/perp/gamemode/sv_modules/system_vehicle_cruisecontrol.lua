...
function GM.ToggleCruiseControl(objPl, Key)
	...
	if(Key == IN_USE and IsValid(vehicle)) then
		objPl:ConCommand("-forward")
		objPl:ConCommand("-back")
		objPl:ConCommand("-jump")
	end
	...
	if(vehicle.CruiseControl == nil) then
		...
		vehicle:SetSharedInt("CruiseControlValue", vehicle.CruiseControlSpeed)
		if(vehicle.CruiseControl) then
			vehicle:SetSharedInt("CruiseControl", 1)
		else
			vehicle:SetSharedInt("CruiseControl", 0)
		end
	end
	...
	if(Key == IN_ATTACK2) then
		...
		if(vehicle.CruiseControl) then
			...
			vehicle:SetSharedInt("CruiseControlValue", 0)
...
timer.Create("CruiseControlTimer", 0.075, 0, function() GAMEMODE.CruiseControlTimer() end)
