...
local function getRealChatOOC ( UMsg )
	...
	local glowType;
	if (pl:IsOwner() && pl:GetSharedBool("disguise", false) != true) then
		glowType = Color(255, 0, 0)
	elseif (pl:IsSuperAdmin() && pl:GetSharedBool("disguise", false) != true) then
		glowType = Color(128, 0, 128)
	elseif (pl:IsAdmin() && pl:GetSharedBool("disguise", false) != true) then
...
GM.chatPrefixes["drop"] = "Amount $";
GM.chatPrefixes["barf"] = "Throw Up";
GM.chatPrefixes["nlr"] = "Check NLR status";
