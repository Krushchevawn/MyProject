...
function PLAYER:SwapItemPosition ( first, second )	
	...
	if (!self:Alive()) then self:Notify("You cannot manipulate items while dead."); return false; end
	if (CLIENT && GAMEMODE.UnarrestTime) then LocalPlayer():Notify("You cannot manipulate items while in jail."); return false; end
	if (SERVER && self.CurrentlyJailed) then LocalPlayer():Notify("You cannot manipulate items while in jail."); return false; end
	...
	if ( second < 3 && ( !firstB.Table.EquipZone || firstB.Table.EquipZone != second ) ) then
		if CLIENT then
	...
	if ( first < 3 && haveSecond && ( !secondB.Table.EquipZone || secondB.Table.EquipZone != first ) ) then
		if CLIENT then
			surface.PlaySound(ThatDoesntGoThere);
			GAMEMODE.InventoryBlocks_Linear[first]:GrabItem();
			GAMEMODE.InventoryBlocks_Linear[second]:GrabItem();
	...
	if SERVER && self:Team() == TEAM_CITIZEN then
		if (second <= 2 && firstB.Table.Equip) then
			local pistol = self:GetAmmoCount('pistol');
			local buckshot = self:GetAmmoCount('buckshot');
			local rifle = self:GetAmmoCount('smg1');
			...
			self:SetAmmo(pistol, 'pistol');
			self:SetAmmo(buckshot, 'buckshot');
			self:SetAmmo(rifle, 'smg1');
