...
function PLAYER:GetOrganizationName ( )
	if (self:GetSharedInt("org", 0) == 0) then return nil; end
	
	if (GAMEMODE.OrganizationData[self:GetSharedInt("org", 0)] && GAMEMODE.OrganizationData[self:GetSharedInt("org", 0)][1] != "fa") then
		if (GAMEMODE.OrganizationData[self:GetSharedInt("org", 0)][1] == "f") then
			return nil;
		else
			return GAMEMODE.OrganizationData[self:GetSharedInt("org", 0)][1];
		end
	end
	
	RunConsoleCommand("perp_srod", self:GetSharedInt("org", 0));
	GAMEMODE.OrganizationData[self:GetSharedInt("org", 0)] = {"f"};
