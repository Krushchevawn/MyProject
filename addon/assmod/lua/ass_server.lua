...
function CONSOLE:IsBetterOrSame(PL2)	if (!self:IsValid()) then return ASS_LVL_SERVER_OWNER <= PL2:GetSharedInt("ASS_isAdmin") else return ASS_LVL_GUEST <= PL2:GetSharedInt("ASS_isAdmin") end end
...
function PLAYER:SetLevel( RANK )	
	self:SetSharedInt("ASS_isAdmin", RANK )
...
function PLAYER:SetTAExpiry(TIME)	
	self:SetSharedFloat("ASS_tempAdminExpiry", TIME)
end
...
function ASS_TellPlayers( PLAYER, ACL, ACTION, ALLPLAYERS )
	...
	for _, pl in pairs(player.GetAll()) do
		...
		elseif(!ALLPLAYERS and pl:IsModerator()) then
			pl:ChatPrint( PLAYER:Nick() .. ": " .. ACTION )
		end
	end
end
...
function ASS_LogAction( PLAYER, ACL, ACTION, ALLPLAYERS )
	Msg( PLAYER:Nick() .. " -> " .. ACTION .. "\n")


	ASS_TellPlayers(PLAYER, ACL, ACTION, ALLPLAYERS)
...
function ASS_PlayerInitialSpawn( PLAYER )
	...
	PLAYER:SetSharedString("ASS_UniqueID", PLAYER:UniqueID())
	if (ASS_IsLan()) then 
		PLAYER:SetSharedString("ASS_AssID", PLAYER:IPAddress())
	else 
		PLAYER:SetSharedString("ASS_AssID", PLAYER:SteamID())
	end 
	
	ASS_LogAction(PLAYER, ASS_ACL_JOIN_QUIT, "entered the server.")
...
function ASS_FindPlayerName( NAME )
	...
	for _,pl in pairs(player.GetAll()) do
		local name = pl:Nick()
		if (string.find(string.lower(name), lNAME)) then
			table.insert(fplys, pl)
		end
	end
	...
	if (CurTime() >= NextDateTimeSet) then 
		...
		for k, v in pairs(PlayerRankings) do
			if (v.Rank == ASS_LVL_BANNED && v.UnbanTime && timeExpired(v.UnbanTime) ) then
				...
				PlayerRankings[ k ].Rank = ASS_LVL_GUEST
				PlayerRankings[ k ].UnbanTime = nil
				PlayerRankings[ k ].BannedBy = nil
				PlayerRankings[ k ].BanTime = nil
				PlayerRankings[ k ].BannedReason = nil
				ASS_SaveRankings();
	...
	if (PLAYER:IsAdmin()) then
		...
		if (ASS_RunPluginFunction( "AllowPlayerUnBan", true, PLAYER, IS_IP, ID_OR_IP )) then
			...
			if (PlayerRankings[ ID_OR_IP ]) then
				...
				PlayerRankings[ ID_OR_IP ].BannedBy = nil
				PlayerRankings[ ID_OR_IP ].BannedReason = nil
				PlayerRankings[ ID_OR_IP ].BanTime = nil
				
				ASS_SaveRankings();
			end

			local name = PlayerRankings[ ID_OR_IP ].Name
			if(!name) then name = "Unknown" end

			if (IS_IP) then
				ASS_LogAction( PLAYER, ASS_ACL_BAN_KICK, "unbanned IP Address \"" .. ID_OR_IP .. "\"", true )
			else
				ASS_LogAction( PLAYER, ASS_ACL_BAN_KICK, "unbanned " .. name .. " (" .. ID_OR_IP .. ")", true )
			end
...
function ASS_BanPlayer( PLAYER, UNIQUEID, TIME, REASON )
	...
	if (#REASON == 0) then REASON = "Banned: No Reason Specified" end
	...
	if (PLAYER:IsTempAdmin()) then
		...
		if (!TO_BAN) then
			local st = UNIQUEID
			if(string.Left(string.upper(st), 5) == "STEAM") then
				local steamid = string.upper(st)
				DataBase:Query("SELECT `steamid` FROM `perp_users` WHERE `id`='" .. steamid .. "'", function ( info )
					local name = "Unknown"
					if(info and info[1] and info[1][1]) then
						name = info[1][1]
					end

					if((PlayerRankings[ st ] and PlayerRankings[ st ].Rank > PLAYER:GetLevel()) or !PlayerRankings[st]) then
						PlayerRankings[ st ] = PlayerRankings[ ID ] or {}
						PlayerRankings[ st ].Rank = ASS_LVL_BANNED
						PlayerRankings[ st ].Name = name
						PlayerRankings[ st ].PluginValues = PlayerRankings[ st ].PluginValues or {}
						PlayerRankings[ st ].UnbanTime = nil
						PlayerRankings[ st ].BanTime = TIME
						PlayerRankings[ st ].BannedBy = PLAYER:Nick()
						PlayerRankings[ st ].BannedReason = REASON
						
						
						if (TIME > 0) then		
							PlayerRankings[ st ].UnbanTime = timeAdd( TIME )
							print(timeAdd( TIME ))
						end
						
						asscmd.ConsoleCommand("banid " .. TIME .. " " .. st .. "\n")
						asscmd.ConsoleCommand("writeid\n")
						ASS_SaveRankings()

						if(TIME == 0) then
							ASS_LogAction( PLAYER, ASS_ACL_BAN_KICK, "banned " .. name .. " (" .. st .. ") permanently. Reason: " .. REASON, true )
						else
							ASS_LogAction( PLAYER, ASS_ACL_BAN_KICK, "banned " .. name .. " (" .. st .. ") for " .. TIME .. " minutes. Reason: " .. REASON, true )
						end
					else
						ASS_MessagePlayer(PLAYER, "Player is a higher rank then you!\n")
					end
    			end)
			else
				ASS_MessagePlayer(PLAYER, "Player not found!\n")
		...
		if (ASS_RunPluginFunction( "AllowPlayerBan", true, PLAYER, TO_KICK, TIME, REASON )) then
			...
			PlayerRankings[ TO_BAN:SteamID() ].BannedBy = PLAYER:Nick()
			PlayerRankings[ TO_BAN:SteamID() ].BannedReason = REASON
			PlayerRankings[ TO_BAN:SteamID() ].BanTime = TIME

			if (TIME > 0) then		

				TO_BAN:SetUnBanTime( timeAdd( TIME ) )

				ASS_LogAction( PLAYER, ASS_ACL_BAN_KICK, "banned " .. ASS_FullNick(TO_BAN) .. " for " .. TIME .. " minutes. Reason: " .. REASON, true )
			else
				ASS_LogAction( PLAYER, ASS_ACL_BAN_KICK, "banned " .. ASS_FullNick(TO_BAN) .. " permanently. Reason: " .. REASON, true )
			end
			...
			if(TIME != 0) then
				asscmd.ConsoleCommand("kickid " .. TO_BAN:UserID() .. " \"" .. REASON ..  " (Banned for " .. TIME .. " minutes)" .. "\"\n")
			else
				asscmd.ConsoleCommand("kickid " .. TO_BAN:UserID() .. " \"" .. REASON ..  " (Banned Permanently)" .. "\"\n")
			end
...
function ASS_KickPlayer( PLAYER, UNIQUEID, REASON )
	...
	if (PLAYER:IsModerator()) then
		...
		if (ASS_RunPluginFunction( "AllowPlayerKick", true, PLAYER, TO_KICK, REASON )) then
			...
			ASS_LogAction( PLAYER, ASS_ACL_BAN_KICK, "kicked " .. ASS_FullNick(TO_KICK) .. " with reason \"" .. REASON .. "\"", true )
			...
			for id, entry in pairs(PlayerRankings) do
				if (entry.Rank == ASS_LVL_BANNED) then
					umsg.Start("ASS_BannedPlayer", pl)
						...
						umsg.String( id )
						umsg.String(entry.BannedBy)
						umsg.String(entry.BannedReason)
						umsg.Long(entry.BanTime)
					umsg.End()
				end
			end
